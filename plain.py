...
class EncryptedFS(Fuse):
    ...
    def mkdir(self, path, mode):
        """
        Create a directory at the given path with the specified mode.
        :param path: Path to the new directory.
        :param mode: Permissions for the new directory.
        """
        logging.debug(f"mkdir called for path: {path}, mode: {oct(mode)}")

        # Remove the leading '/' from the path to get the directory name
        dirname = path.lstrip("/")

        # Check if the directory already exists
        if dirname in self.file_data:
            logging.error(f"mkdir: Directory already exists: {dirname}")
            raise fuse.FuseOSError(errno.EEXIST)

        # Create a new directory
        dir_stat = FileStat()
        dir_stat.st_mode = stat.S_IFDIR | mode  # Directory mode
        dir_stat.st_nlink = 2  # Two links for the new directory: itself and its parent
        dir_stat.st_size = 0  # Directories have no size

        self.file_data[dirname] = FileData(b"", cipher=None)  # Directories don't need encryption
        self.file_data[dirname].stat = dir_stat
    ...
    def readdir(self, path: str, offset: int):
        ...
        entries = ['.', '..'] + list(self.file_data.keys())
    ...
    def getattr(self, path, fh=None):
        ...
        if filename in self.file_data:
            ...
            if file_stat.st_mode & stat.S_IFDIR:
                file_stat.st_mode = stat.S_IFDIR | 0o755
            else:
                file_stat.st_mode = stat.S_IFREG | 0o644
